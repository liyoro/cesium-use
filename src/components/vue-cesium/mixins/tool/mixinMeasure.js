...
const methods = {
  ...
  getHeight(endPoint, polyline) {
    ...
    const planeStart = new Cesium.Plane(
      normalStart,
      -Cesium.Cartesian3.distance(this.startPoint, new Cesium.Cartesian3(0, 0, 0))
    )
    const hypPoint = {}
    ...
    if (polyline.height <= 0) {
      ...
      const planeEnd = new Cesium.Plane(
        normalStart,
        -Cesium.Cartesian3.distance(endPoint, new Cesium.Cartesian3(0, 0, 0))
      )
    ...
    const labelTextHeight =
      polyline.height > 1000 ? (polyline.height / 1000).toFixed(2) + 'km' : polyline.height.toFixed(2) + 'm'
    labels.push({
  ...
  onMeasureEvt(polyline, index, flag = false) {
    if (!this.depthTest) {
      ...
      if (type === 'distanceMeasuring' || type === 'areaMeasuring') {
        ...
        listener &&
          this.$emit(listener, { polyline: polyline, label: labelsResult, type: 'heightMeasuring', finished: flag })
      }
    }
  },
