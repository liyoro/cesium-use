...
export default {
  ...
  methods: {
    handleZoomBarScrollMouseDown(event, viewModel = this) {
      ...
      defined(viewModel.zoombarTickFunction) &&
        viewModel.viewer.clock.onTick.removeEventListener(viewModel.zoombarTickFunction)
      ...
      viewModel.zoomBarScrollMouseMoveFunction = e => {
        ...
        const endPosition = {}
        endPosition.x =
          e.type === 'touchmove' ? e.changedTouches[0].clientX - rectNavigation.left : e.clientX - rectNavigation.left
        endPosition.y =
          e.type === 'touchmove' ? e.changedTouches[0].clientY - rectNavigation.top : e.clientY - rectNavigation.top
    ...
    handleZoomInMouseDown(event, viewModel = this) {
      ...
      defined(viewModel.zoominTickFunction) &&
        viewModel.viewer.clock.onTick.removeEventListener(viewModel.zoominTickFunction)
    ...
    handleZoomOutMouseDown(event, viewModel = this) {
      const { defined, getTimestamp, SceneMode, ScreenSpaceEventType } = Cesium
      viewModel.screenSpaceEventHandler.removeInputAction(ScreenSpaceEventType.LEFT_UP)
      defined(viewModel.zoomoutTickFunction) &&
        viewModel.viewer.clock.onTick.removeEventListener(viewModel.zoomoutTickFunction)
