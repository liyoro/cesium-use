...
function rotate(viewModel, compassElement, cursorVector) {
  ...
  if (!defined(viewCenter)) {
    viewModel.rotateFrame = Transforms.eastNorthUpToFixedFrame(
      camera.positionWC,
      viewModel.viewer.scene.globe.ellipsoid,
      newTransformScratch
    )
...
function tilt(viewModel, compassElement, cursorVector) {
  ...
  tiltInitialCursorAngle =
    tiltInitialCursorAngle > CesiumMath.PI_OVER_TWO ? CesiumMath.PI_OVER_TWO : tiltInitialCursorAngle
  ...
  defined(pickPosition) &&
    (viewModel.tiltFrame = Transforms.eastNorthUpToFixedFrame(pickPosition, scene.globe.ellipsoid))
